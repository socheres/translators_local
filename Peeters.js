{
	...
	"target": "^http?://(www\\.)?poj.peeters-leuven.be/content.php",
	"minVersion": "3.0",
	"maxVersion": "",
	"priority": 100,
	"inRepository": true,
...
function scrape(doc, url) {
	...
	for (let name of authors) { 
		if (ALLCaps(name)) { 
			if (creator.lastName === undefined) { 
				creator.lastName = ZU.capitalizeTitle(name, true); 
			}
			else {
	...
	var issue = ZU.xpathText(doc, '//b[contains(text(), "Issue:")]/following-sibling::text()[1]');
	if (issue) {
		item.issue = issue;
	}
	var date = ZU.xpathText(doc, '//b[contains(text(), "Date:")]/following-sibling::text()[1]');
	if (date) {
		item.date = date;
	}
	var pages = ZU.xpathText(doc, '//b[contains(text(), "Pages:")]/following-sibling::text()[1]');
	if (pages) {
		item.pages = pages;
	}
	var doi = ZU.xpathText(doc, '//b[contains(text(), "DOI:")]/following-sibling::text()[1]');
	if (doi) {
		item.DOI = doi;
	}
